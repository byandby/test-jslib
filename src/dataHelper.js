...
const wrapperSaga = config => {
  return function* wrappered(action) {
    yield put({ type: UPDATE_DATA_STATUS, key: config.type, status: DataStatus.FETCHING })
    ...
    try {
      ...
      yield put({ type: UPDATE_DATA_STATUS, key: config.type, status: DataStatus.SUCCESS })
