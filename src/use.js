...
import { DataStatus } from './dataHelper'
...
export const useApi = ({ fn, onSuccess, onFail }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        result = { status: DataStatus.SUCCESS, response }
        if (typeof onSuccess === 'function') onSuccess(response)
      } catch (error) {
        result = { status: DataStatus.FAILED, response: error }
  ...
  const handler = param => {
    setState({
      ...state,
      param,
      result: { status: DataStatus.FETCHING },
      init: false,
